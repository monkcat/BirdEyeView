...
"""
def process_camera(idx, img, calibration_data, distances, lock, config, roi, point, lut):
    # 2. ??? ?? ??
...
def process_camera(idx, frame_queue, calibration_data, distances, lock, roi, point, lut):
    while True:
        # 큐에서 데이터 가져오기
...
if __name__ == '__main__':
    ...
    device_list = []
    for config in camera_configs:
        device_list.append(config['device'])

    frame_queues = [Queue(maxsize = 1) for _ in device_list]
    threads = [CameraThread(camera_id, frame_queues[i]) for i, camera_id in enumerate(device_list)]
    ...
    for i, config in enumerate(camera_configs):
        ...
        p = Process(target=process_camera, args=(idx, frame_queues[i], calib_data, distances, lock, config, roi, point, lut))
        processes.append(p)
        p.start()
    ...
    print(f"lut time : {lut_time - start}, process time : {end_time - cap_time}, cap_time : {cap_time - lut_time}, total_time : {end_time - start}")
